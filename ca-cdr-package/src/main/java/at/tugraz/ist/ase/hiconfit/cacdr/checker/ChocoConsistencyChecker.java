...
public class ChocoConsistencyChecker implements IConsistencyChecker {
    ...
    public boolean isConsistent(@NonNull Collection<Constraint> C) {
        ...
        ConstraintUtils.postConstraints(C, model);
    ...
    public boolean isConsistent(@NonNull Collection<Constraint> C, @NonNull Constraint cstr) {
        ...
        // post constraints of the parameter C
        ConstraintUtils.postConstraints(CF, model);

        // ∪ {¬cstr}
        ConstraintUtils.postConstraint(cstr, model, true);
    ...
    public void reset() {
        ...
        incrementCounter(ConstraintUtils.COUNTER_UNPOST_CONSTRAINT, model.getNbCstrs());
...
//            }
    ...
    protected void postTestCase(TestCase testcase, boolean negative) {
        if (!negative) {
            ...
            incrementCounter(ConstraintUtils.COUNTER_POST_CONSTRAINT, testcase.getChocoConstraints().size());
            log.trace("{}Added test case's constraints", LoggerUtils.tab());
        } else {
            testcase.getNegChocoConstraints().forEach(model::post);
            incrementCounter(ConstraintUtils.COUNTER_POST_CONSTRAINT, testcase.getNegChocoConstraints().size());

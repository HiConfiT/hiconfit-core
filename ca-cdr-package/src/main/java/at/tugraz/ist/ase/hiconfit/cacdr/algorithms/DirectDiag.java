...
public class DirectDiag extends IConsistencyAlgorithm {
    ...
    public Set<Constraint> findDiagnosis(@NonNull Set<Constraint> C, @NonNull Set<Constraint> B) {
        ...
        Set<Constraint> BwithC = Sets.union(B, C); PerformanceEvaluator.incrementCounter(CAEvaluator.COUNTER_UNION_OPERATOR);
        ...
        if (C.isEmpty()
            ...
            PerformanceEvaluator.incrementCounter(COUNTER_DIRECTDIAG_CALLS);
            PerformanceEvaluator.start(TIMER_DIRECTDIAG);
            Set<Constraint> diag = dd(Collections.emptySet(), C, B);
            PerformanceEvaluator.stop(TIMER_DIRECTDIAG);
    ...
    private Set<Constraint> dd(Set<Constraint> D, Set<Constraint> C, Set<Constraint> B) {
        ...
        // Δ1 = DD(C1, C1, B);
        PerformanceEvaluator.incrementCounter(CAEvaluator.COUNTER_LEFT_BRANCH_CALLS);
        PerformanceEvaluator.incrementCounter(COUNTER_DIRECTDIAG_CALLS);
        Set<Constraint> Δ1 = dd(C1, C1, B);

        // Δ2 = DD(Δ1, C2, B U C1 - Δ1);
        Set<Constraint> BwithC1 = Sets.union(B, C1); PerformanceEvaluator.incrementCounter(CAEvaluator.COUNTER_UNION_OPERATOR);
        Set<Constraint> BwithC1withoutΔ1 = Sets.difference(BwithC1, Δ1); PerformanceEvaluator.incrementCounter(CAEvaluator.COUNTER_DIFFERENT_OPERATOR);
        PerformanceEvaluator.incrementCounter(CAEvaluator.COUNTER_RIGHT_BRANCH_CALLS);
        PerformanceEvaluator.incrementCounter(COUNTER_DIRECTDIAG_CALLS);
        ...
        // return Δ1 ∪ Δ2;
        PerformanceEvaluator.incrementCounter(CAEvaluator.COUNTER_UNION_OPERATOR);
